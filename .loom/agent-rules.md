...
7. 실행은 승인됐지만 현재 에이전트에게 직접 맡기지 않았다면 Task를 Queue에 등록하고 Worker가 실행하게 합니다.
8. 사용자가 실행을 승인하지 않았다면 계약 작성과 보완만 수행하며 Task를 실행하거나 Queue에 등록하지 않습니다.
9. 완료된 Task는 재실행하지 않습니다. 완료된 Job의 후속 작업이 같은 Job 목표 안에 있으면 새 Task로 추가하고, 목표가 다르면 새 Job을 제안합니다.
10. 에이전트가 자신의 권한 요청을 승인하거나 거절된 정책을 우회하지 않습니다.
11. 새로 발견한 작업을 현재 Task에 몰래 확장하지 않고 후속 Proposal로 분리합니다.
12. 작업 중 유의미한 원칙, 판단, 후속 작업을 발견하면 workflow 상태로 몰래 확정하지 않고 Memory Proposal 후보로 사용자에게 제안합니다.
13. 작업 종료 전 metadata를 검증하고 결과, 검증, 남은 위험, 새 제안과 다음 행동을 보고합니다.
...
- Implemented process adapters: `local`, `codex`, `codex:<model>`, `codex:<model>:<reasoning>`
- Planned process adapters: `claude-code`
- Claude Code는 현재 Loom CLI를 사용하는 foreground 통제 에이전트로 사용할 수 있습니다. 전용 Claude Code process adapter는 별도 후속 구현입니다.
...
- `.loom/docs/README.md`: init이 생성하는 Loom 내부 동작 문서의 색인입니다.
- `.loom/docs/agent-workflow.md`: 통제 에이전트, 작업 경계, 실행 경로를 정의하는 운영 계약 문서입니다.
- `.loom/docs/context-pack.md`: Task context pack의 포함/제외 기준과 required docs, memory refs 규칙입니다.
- `.loom/docs/memory-profile.md`: workflow memory 저장 정책, proposal lifecycle, 사용자 확정 기억 규칙입니다.
